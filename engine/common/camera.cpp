...
float Camera::zNear() const {
  if(vrViewActive) return 2.f;
  static float near = 10.f;
  return near;
  }

static float vrFarPlane = 100000.0f;

void Camera::setVrFarPlane(float cm) {
  vrFarPlane = cm;
  }

float Camera::zFar() const {
  if(vrViewActive) return vrFarPlane;
...
Matrix4x4 Camera::projective() const {
  if(vrViewActive) return vrProjection;
  auto ret = proj;
...
Matrix4x4 Camera::viewShadowVsmLwc(const Tempest::Vec3& ldir) const {
  return mkViewShadowVsm(inter.target-originLwc(),ldir);
  }
...
Matrix4x4 Camera::viewShadowLwc(const Tempest::Vec3& lightDir, size_t layer) const {
  ...
  return mkViewShadow(inter.target-originLwc(),rotation,vp,lightDir,layer);
  }
...
Matrix4x4 Camera::vrBaseView(const Vec3& position,float yaw) const {
  return mkView(position,Vec3(0,yaw,0));
}

Matrix4x4 Camera::view() const {
  if(vrViewActive) return vrView;
...
void Camera::setVrView(const Matrix4x4& view,const Matrix4x4& projection) {

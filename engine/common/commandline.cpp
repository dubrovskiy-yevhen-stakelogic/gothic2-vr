...
CommandLine::CommandLine(int argc, const char** argv) {
  ...
  for(int i=1;i<argc;++i) {
    ...
    if(arg.find("-game:")==0) {
      ...
      if(++i>=argc)
        throw std::invalid_argument("-fps requires 0..240");
      std::string value = argv[i];
      size_t parsed = 0;
      fps = std::stoi(value, &parsed);
      if(parsed!=value.size() || fps<0 || fps>240)
        throw std::invalid_argument("-fps requires 0..240");
      }
  ...
  if(smoke) {
    if(gpath.empty() || !saveDef.empty() || !mod.empty() || isBenchmark!=Benchmark::None)
      throw std::invalid_argument("-baseline-smoke requires -g and a new unmodified game");
    noMenu = true;
    isWindow = true;
    if(fps<0)
      fps = 60;
    if(!smokeFixture.empty()) saveDef = smokeFixture;
    }

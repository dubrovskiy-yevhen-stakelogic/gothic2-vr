...
bool DrawBuckets::enableIndexed(uint32_t bucketId) {
  if(bucketId>=bucketsCpu.size()) return false;
  const auto& bucket=bucketsCpu[bucketId];
  const auto& indices=bucket.staticMesh ? bucket.staticMesh->indexedSource : bucket.animMesh->indexedSource;
  const auto& ibo=bucket.staticMesh ? bucket.staticMesh->ibo : bucket.animMesh->ibo;
  if(indices.empty() || indices.size()!=ibo.size()) return false;
  auto found=indexedMeshes.find(&ibo);
  if(found==indexedMeshes.end()) {
    if(indices.size()>UINT32_MAX || indexedPoolCpu.size()>UINT32_MAX-indices.size()) return false;
    const IndexedRange range{uint32_t(indexedPoolCpu.size()),uint32_t(indices.size())};
    indexedPoolCpu.insert(indexedPoolCpu.end(),indices.begin(),indices.end());
    found=indexedMeshes.emplace(&ibo,range).first;
    indexedPoolDirty=true;
  }
  if(indexedRangesCpu.size()<bucketsCpu.size()) indexedRangesCpu.resize(bucketsCpu.size());
  const auto& range=found->second;
  if(indexedRangesCpu[bucketId].count!=range.count || indexedRangesCpu[bucketId].first!=range.first) {
    indexedRangesCpu[bucketId]=range;
    indexedRangesDirty=true;
  }
  return true;
}

bool DrawBuckets::commit(Encoder<CommandBuffer>& cmd, uint8_t fId) {
  auto& gpuDevice=Resources::device();
  if(indexedPoolDirty) {
    indexedPoolDirty=false;
    Resources::recycle(std::move(indexedPoolGpu));
    indexedPoolGpu=gpuDevice.ibo(indexedPoolCpu);
    Log::i("VR indexed pool bytes=",indexedPoolGpu.byteSize()," uniqueMeshes=",indexedMeshes.size());
  }
  if(indexedRangesDirty) {
    indexedRangesDirty=false;
    Resources::recycle(std::move(indexedRangesGpu));
    indexedRangesGpu=gpuDevice.ssbo(indexedRangesCpu);
  }
  if(!bucketsDurtyBit)
    return false;

...
uint32_t DrawClusters::alloc(const PackedMesh::Cluster* cx, size_t firstMeshlet, size_t meshletCount, uint16_t bucketId, uint16_t commandId, uint8_t lod) {
  ...
  for(size_t i=0; i<meshletCount; ++i) {
    ...
    c.meshletCount = 1u | (uint32_t(lod)<<24);
...
void DrawClusters::patchClusters(Encoder<CommandBuffer>& cmd, uint8_t fId) {
  ...
  invalidateCpu();
  auto& header=scratch.header;
  auto& patch=scratch.patch;
  if(header.empty())
    return;
  ...
  std::fill(clustersDurty.begin(),clustersDurty.end(),0u);
  }

void DrawClusters::prepareCpu() {
  invalidateCpu();
  dirtySnapshot=clustersDurty;
  ...
  for(size_t i=0; i<dirtySnapshot.size(); ++i) {
    if(dirtySnapshot[i]==0x0)
      continue;
    const uint32_t mask = dirtySnapshot[i];
  ...
  // Live bits remain set until the GPU commit accepts this snapshot.
...
void DrawClusters::markClusters(size_t id, size_t count) {
  ...
  for(size_t i=0; i<count; ++i) {
    ...
    __atomic_fetch_or(&bits,1u << (id%32),__ATOMIC_RELAXED);
#else
    std::atomic_ref<uint32_t>(bits).fetch_or(1u << (id%32),std::memory_order_relaxed);
#endif
    id++;
    }

...
void Frustrum::make(const Matrix4x4& m, int32_t w, int32_t h, DepthRange depthRange) {
  width  = uint32_t(w);
  height = uint32_t(h);
  mat    = m;
  ...
  // Vulkan/D3D clip at z=0, including reverse-depth shadow maps. Using
  // w+z here would admit an extra slab that the rasterizer later clips.
  const float clipW = depthRange==DepthRange::ZeroToOne ? 0.f : 1.f;
  f[5][0] = clipW*clip[ 3] + clip[ 2];
  f[5][1] = clipW*clip[ 7] + clip[ 6];
  f[5][2] = clipW*clip[11] + clip[10];
  f[5][3] = clipW*clip[15] + clip[14];

...
void MdlVisual::setRenderVisible(bool visible) {
  view.setVisible(visible);
  for(auto* mesh:{&head,&sword,&shield,&bow,&ammunition,&stateItm}) mesh->view.setVisible(visible);
  for(auto& mesh:item) mesh.view.setVisible(visible);
  for(auto& mesh:attach) mesh.view.setVisible(visible);
}

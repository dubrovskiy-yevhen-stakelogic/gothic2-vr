...
StaticMesh::StaticMesh(const PackedMesh& mesh, bool retainIndexedSource) {
#if defined(__ANDROID__)
  if(retainIndexedSource && Gothic::options().doBindless && Resources::device().properties().indirect.indexed &&
     Resources::device().properties().indirect.count && Resources::device().properties().indirect.multiDraw &&
     Resources::device().properties().indirect.firstInstance)
    indexedSource=mesh.indices;
#else
  (void)retainIndexedSource;
#endif
...
#if defined(__ANDROID__)
  if(Gothic::options().doBindless && Resources::device().properties().indirect.indexed &&
     Resources::device().properties().indirect.count && Resources::device().properties().indirect.multiDraw &&
     Resources::device().properties().indirect.firstInstance)
    indexedSource=std::move(cibo);
#endif

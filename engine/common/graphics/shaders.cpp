...
void Shaders::compileShaders() {
  ...
  visibilityPassHiZSeed = computeShader("visibility_pass_hiz_seed.comp.sprv");
#endif
...
#if defined(__ANDROID__)
  directAmbient   = postEffect("direct_ambient",    RenderState::ZTestMode::NoEqual);
  directAmbientSh = postEffect("direct_ambient_sh", RenderState::ZTestMode::NoEqual);
  directLightShTiles   = postEffect("direct_light_sh_tiles",   RenderState::ZTestMode::NoEqual);
  directAmbientShTiles = postEffect("direct_ambient_sh_tiles", RenderState::ZTestMode::NoEqual);
  shadowTiles          = computeShader("shadow_tiles.comp.sprv");
  unlit           = postEffect("unlit", RenderState::ZTestMode::NoEqual);
#endif
...
#if defined(__ANDROID__)
  vrFogProbe         = computeShader("vr_fog_probe.comp.sprv");
  skyExposureStore   = computeShader("sky_exposure_store.comp.sprv");
  skyExposureApply   = computeShader("sky_exposure_apply.comp.sprv");
  indexedObjects     = computeShader("indexed_objects.comp.sprv");
  indexedBindless    = computeShader("indexed_bindless.comp.sprv");
  lightVisibilityInit = computeShader("light_visibility_init.comp.sprv");
  lightVisibility     = computeShader("light_visibility.comp.sprv");
  lightTiles          = computeShader("light_tiles.comp.sprv");
  lightTilesSlab      = computeShader("light_tiles_slab.comp.sprv");
  const auto& subgroup=device.properties().subgroup;
  if(subgroup.compute && subgroup.basic && subgroup.arithmetic)
    lightTilesSubgroup = computeShader("light_tiles_subgroup.comp.sprv");
#endif
...
#if defined(__ANDROID__)
  sh = GothicShader::get("light_reference.vert.sprv");
  auto vsReference = device.shader(sh.data,sh.len);
  lightsReference = device.pipeline(Triangles, state, vsReference, fsLight);
  {
    auto farState=state;
    farState.setCullFaceMode(RenderState::CullMode::Back);
    farState.setZTestMode(RenderState::ZTestMode::GEqual);
    sh=GothicShader::get("light_far.vert.sprv");
    auto vsFar=device.shader(sh.data,sh.len);
    lightsFar=device.pipeline(Triangles,farState,vsFar,fsLight);
  }
  {
    auto tiledState=state;
    tiledState.setCullFaceMode(RenderState::CullMode::NoCull);
    tiledState.setZTestMode(RenderState::ZTestMode::NoEqual);
    sh=GothicShader::get("triangle.vert.sprv");
    auto vsTiled=device.shader(sh.data,sh.len);
    sh=GothicShader::get("light_tiled.frag.sprv");
    auto fsTiled=device.shader(sh.data,sh.len);
    lightsTiled=device.pipeline(Triangles,tiledState,vsTiled,fsTiled);
    // Light slabs: same fullscreen triangle, mask-driven fragment shader.
    sh=GothicShader::get("light_tiled_slab.frag.sprv");
    auto fsSlab=device.shader(sh.data,sh.len);
    lightsSlab=device.pipeline(Triangles,tiledState,vsTiled,fsSlab);
    if(device.properties().ubo.maxRange>=2048u*32u) {
      sh=GothicShader::get("light_tiled_uniform.frag.sprv");
      auto fsUniform=device.shader(sh.data,sh.len);
      lightsTiledUniform=device.pipeline(Triangles,tiledState,vsTiled,fsUniform);
      sh=GothicShader::get("light_tiled_slab_uniform.frag.sprv");
      auto fsSlabUniform=device.shader(sh.data,sh.len);
      lightsSlabUniform=device.pipeline(Triangles,tiledState,vsTiled,fsSlabUniform);
    }
  }
#endif
  ...
  tonemappingFog     = postEffect("triangle_uv", "tonemapping_fog", RenderState::ZTestMode::Always);
  ...
  hiZStereo = computeShader("hiz_stereo.comp.sprv");
...
const RenderPipeline* Shaders::materialPipeline(const Material& mat, DrawCommands::Type t, PipelineType pt, bool bl, bool indexed) const {
  ...
  for(auto& i:materials) {
    if(i.alpha==alpha && i.type==t && i.pipelineType==pt && i.bindless==bl && i.trivial==trivial && i.indexed==indexed)
      return &i.pipeline;
    }
  ...
  if(mat.isTesselated() && device.properties().tesselationShader && t==DrawCommands::Landscape && true) {
    ...
    auto shVs = GothicShader::get(string_frm("main_", vsTok, typeVs, bindless, indexed ? "_indexed" : "", ".vert.sprv"));
    auto shFs = GothicShader::get(string_frm("main_", fsTok, typeFs, bindless, ".frag.sprv"));

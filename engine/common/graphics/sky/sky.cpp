...
void Sky::setVrWeather(int mode) {
  weather=std::clamp(mode,0,3);
  // Transparent Color() is a sentinel returning nullptr in Resources.
  const auto loaded=weather==1?nullptr:Resources::loadTexture(Color(.45f,.48f,.52f,1));
  const auto texture=loaded?loaded:&Resources::fallbackBlack();
  weatherClouds.lay[0]=weatherClouds.lay[1]=texture;
  }
...
void Sky::updateLight(const gtime gameTime) {
  ...
  if(weather>=2) {
    sun.setColor(direct*sunMul*(weather==3?.12f:.22f));
    ambient*=weather==3?.55f:.7f;
  }
  GSunIntensity=DirectSunLux*(weather>=2?.25f:1.f);
  GMoonIntensity=DirectMoonLux*(weather>=2?.3f:1.f);
  }

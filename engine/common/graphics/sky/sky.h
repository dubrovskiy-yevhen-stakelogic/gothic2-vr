...
class Sky final {
  public:
    ...
    void setVrWeather(int mode);
    int vrWeather() const { return weather; }
    ...
    const State&              cloudsDay()   const { return weather==0?clouds[0]:weatherClouds; }
    const State&              cloudsNight() const { return weather==0?clouds[1]:weatherClouds; }

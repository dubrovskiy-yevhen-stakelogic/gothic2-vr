...
std::unique_ptr<ProtoMesh> Resources::implLoadMeshMain(std::string name,bool hideBowString) {
  ...
  if(FileExt::hasExt(name,"MMS") || FileExt::hasExt(name,"MMB")) {
    ...
    if(hideBowString)Log::i("VR bow string triangles removed: ",Vr::removeBowString(zmm.mesh,&stringHeight)," mesh=",name);
    PackedMesh packed(zmm.mesh,PackedMesh::PK_VisualMorph);
    auto mesh=std::make_unique<ProtoMesh>(std::move(packed),zmm.animations,name);
    mesh->vrBowStringHeight=stringHeight;
    return mesh;
    }
...
const ProtoMesh* Resources::loadVrBowMesh(std::string_view name) {
  if(name.empty())return nullptr;
  std::lock_guard<std::recursive_mutex> guard(inst->sync);
  const auto key=std::string("VR_BOW_NO_STRING:")+std::string(name);
  auto found=inst->aniMeshCache.find(key);
  if(found!=inst->aniMeshCache.end())return found->second.get();
  auto mesh=inst->implLoadMeshMain(std::string(name),true);
  auto result=mesh.get();inst->aniMeshCache.emplace(key,std::move(mesh));return result;
}

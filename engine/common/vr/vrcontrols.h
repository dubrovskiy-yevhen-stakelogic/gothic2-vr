...
class Menu {
  public:
    ...
    std::span<const Row> rows() const {
      static constexpr Row main[]={Locomotion,Hud,Performance,Recenter,Holsters,Cheats,Controls,ItemCalibrationMenu,OpenGameInterface,Close};
  ...
  private:
    ...
    void adjust(int direction,bool activated=false) {
      ...
      if(!activated && (selectedRow==CalUseBowDefault || selectedRow==CalUseCrossbowDefault || selectedRow==OpenGameInterface || selectedRow==HolsterMove || selectedRow==HolsterClear || selectedRow==HolsterDrop || selectedRow==HolsterAtLeft || selectedRow==HolsterAtRight || selectedRow==EnemySpawn || selectedRow==ItemGive || selectedRow==Heal || selectedRow==TimeApply || selectedRow==WeatherApply)) return;
      switch(selectedRow) {

...
void Item::setVelocityVr(const Tempest::Vec3& velocity) {
  if(visual.nodesCount()>0) {
    physic=world.physic()->addDynamicObj(transform(),visual.bounds(),zenkit::MaterialGroup(hitem->material),true);
    physic.setItem(this);world.invalidateVobIndex();
  }

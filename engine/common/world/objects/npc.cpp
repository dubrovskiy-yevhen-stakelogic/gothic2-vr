...
#include "diagnostics/gameplayprobe.h"
...
void Npc::commitDamage() {
  if(isPlayer() && vrPhysicalMelee)return; // VR sweeps own player melee damage.
...
void Npc::takeDamage(Npc &other, const Bullet* b, int32_t meleeCap) {
  if(isDown())
    return;

  assert(b==nullptr || !b->isSpell());
  const auto& pose    = visual.pose();
  const bool physicalContact=b==nullptr && other.isPlayer() && other.vrPhysicalMelee;
  const bool isJumpb=!physicalContact && pose.isJumpBack(owner.tickCount()) && fghAlgo.isInJumpBackAngle(*this,other);
  const bool vrBlock=b==nullptr && isPlayer() && vrPhysicalMelee && invent.activeWeapon()!=nullptr &&
                         vrGuard.blocks(other.position(),owner.tickCount());
  if(vrBlock)vrParryFeedback|=1u<<unsigned(vrGuard.hand);
  const bool  isBlock = vrBlock || ((!other.isMonster() || other.inventory().activeWeapon()!=nullptr) &&
  ...
  if(!(isBlock || isJumpb) || b!=nullptr) {
    takeDamage(other,b,COLL_DOEVERYTHING,0,false,meleeCap);
    } else {
    if(physicalContact)Log::i("VR melee blocked target=",displayName()," defence=",isBlock," dodge=",isJumpb);
...
void Npc::takeDamage(Npc& other, const Bullet* b, const CollideMask bMask, int32_t splId, bool isSpell, int32_t meleeCap) {
  ...
  if(b==nullptr && !isSpell)hitResult.value=std::min(hitResult.value,meleeCap);
  if(b==nullptr && !isSpell && other.isPlayer() && other.vrPhysicalMelee)
    Log::i("VR melee result target=",displayName()," hp=",attribute(ATR_HITPOINTS)," damage=",hitResult.value,
           " hit=",hitResult.hasHit," immune=",hitResult.invincible," immortal=",isImmortal()," weapon=",other.activeWeapon()?other.activeWeapon()->displayName():std::string_view("fists"));
  ...
  if(hitResult.value>0) {
    ...
    GameplayProbe::observeDamage(*this,other,previous,attribute(ATR_HITPOINTS));
...
Item* Npc::takeItemVr(Item& item) {
  if(isDown() || interactive()!=nullptr || item.isTorchBurn()) return nullptr;
  auto ptr=owner.takeItem(item);
  if(!ptr) return nullptr;
  auto it=addItem(std::move(ptr));
  if(isPlayer() && it) owner.sendPassivePerc(*this,*this,*it,PERC_ASSESSTHEFT);
  return it;
  }

Item* Npc::dropItemVr(size_t id,const Tempest::Matrix4x4& model,const Tempest::Vec3& velocity) {
  if(!getItem(id) || itemCount(id)==0 || !Vr::finite(velocity))return nullptr;
  auto item=owner.addItemDyn(id,model,hnpc->symbol_index());
  if(!item)return nullptr;
  item->setCount(1);item->setVelocityVr(velocity);
  if(activeWeapon() && activeWeapon()->clsId()==id)closeWeapon(true);
  invent.delItem(id,1,*this);
  return item;
}

void Npc::readyFistsVr() {
  invent.switchActiveWeapon(*this,Item::NSLOT);
  if(visual.setToFightMode(WeaponState::Fist))updateWeaponSkeleton();
  hnpc->weapon=1;
  }
...
bool Npc::equipVr(size_t id,bool ignoreRequirements) {
  ...
  const bool melee=(it->mainFlag()&ITM_CAT_NF)!=0,ranged=(it->mainFlag()&ITM_CAT_FF)!=0;
  if(!melee && !ranged)return false;
  if(!it->isEquipped() && !invent.equip(id,*this,ignoreRequirements))return false;
  invent.switchActiveWeapon(*this,melee?1:2);
  if(!invent.activeWeapon() || invent.activeWeapon()->clsId()!=id)return false;
  const auto state=melee?(it->is2H()?WeaponState::W2H:WeaponState::W1H):(it->isCrossbow()?WeaponState::CBow:WeaponState::Bow);
  if(visual.setToFightMode(state))updateWeaponSkeleton();
  hnpc->weapon=melee?(it->is2H()?4:3):(it->isCrossbow()?6:5);
  return true;
  }

bool Npc::shootVr(const Tempest::Vec3& pos,const Tempest::Vec3& direction,float power,float visualScale) {
  const float length=direction.length();
  if(!std::isfinite(length) || length<.001f || !std::isfinite(power)) return false;
  auto weapon=invent.activeWeapon();
  if(!weapon || (weapon->mainFlag()&ITM_CAT_FF)==0) return false;
  auto ammo=invent.getItem(size_t(weapon->handle().munition));
  if(!ammo || ammo->count()==0) return false;
  const auto id=ammo->clsId();
  auto& bullet=owner.shootBullet(*ammo,*this,nullptr,nullptr);
...
bool Npc::drinkVr(size_t id) {
  auto item=invent.getItem(id);
  if(!item || isDown() || (item->mainFlag()&ITM_CAT_POTION)==0 || !item->checkCond(*this)) return false;
  const auto callback=item->handle().on_state[0];
  if(callback==0) return false;
  const auto before=invent.itemCount(id);
  // Physical drinking replaces the item animation, including its ITEM_DESTROY event.
  // Some scripts consume the potion themselves; never decrement a second time.
  invent.setCurrentItem(id);
  owner.script().invokeItem(this,uint32_t(callback));
  if(invent.itemCount(id)>=before) invent.delItem(id,1,*this);
  return true;
  }
...
void Npc::setVrLocomotionSpeed(float s) {
  mvAlgo.setVrLocomotionSpeed(s);
  }
...
void Npc::updateAnimation(uint64_t dt, bool force) {
  ...
  // NPCs own MdlVisual directly, so they do not pass through ObjVisual's
  // attachment synchronization. Update the head and held objects after bones.
  if(visual.updateAnimation(this,nullptr,owner,dt,force))
    visual.syncAttaches();
  }

...
World::World(GameSession& game, std::string_view file, bool startup, std::function<void(int)> loadProgress)
  ...
  try {
    ...
    auto wviewFut = std::async(std::launch::async, [&]() {
      ...
      PackedMesh vmesh(worldMesh,PackedMesh::PK_VisualLnd,PackedMesh::lodLevelsDefault());
...
Focus World::findFocus(const Npc &pl, const Focus& def, bool unarmed) {
  ...
  if(pl.isPlayer() && vrGaze)for(auto search:{&optNpc,&optMob,&optItm}) {
    search->gaze=true;search->gazeHead=vrGazeHead;search->gazeDirection=vrGazeDirection;
  }

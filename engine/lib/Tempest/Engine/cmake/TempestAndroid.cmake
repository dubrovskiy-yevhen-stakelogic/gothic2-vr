...
# Apply to an application's shared-library target after linking Tempest.
...
function(tempest_android_application name)
  ...
  cmake_parse_arguments(APP "SHRINK_RELEASE;REPACKAGE;ANDROIDX"
  ...
  get_filename_component(engine "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/.." ABSOLUTE)
  set(output "${CMAKE_CURRENT_BINARY_DIR}/${name}")
  file(MAKE_DIRECTORY "${output}/app")
  ...
  endforeach()
  list(PREPEND APP_JAVA_DIRS "${engine}/system/android/java")
  list(PREPEND APP_PROGUARD_FILES "${engine}/system/android/proguard-rules.pro")
  list(PREPEND APP_CMAKE_ARGUMENTS "-DANDROID_STL=c++_static" "-DTEMPEST_BUILD_SHARED=OFF")
  ...
  foreach(value SHRINK_RELEASE REPACKAGE ANDROIDX)
    if(APP_${value})
      set(${value} true)
    else()
      set(${value} false)
  ...
  if(APP_MANIFEST)
    ...
    set(manifest "${output}/app/src/main/AndroidManifest.xml")
  ...
  foreach(file settings.gradle build.gradle gradle.properties app/build.gradle)
    configure_file("${templates}/${file}.in" "${output}/${file}" @ONLY NEWLINE_STYLE LF)
  endforeach()
  foreach(file gradlew gradlew.bat gradle/wrapper/gradle-wrapper.properties)
    configure_file("${templates}/${file}" "${output}/${file}" @ONLY NEWLINE_STYLE LF)
  endforeach()
  configure_file("${templates}/gradle/wrapper/gradle-wrapper.jar" "${output}/gradle/wrapper/gradle-wrapper.jar" COPYONLY)
  if(CMAKE_HOST_WIN32)
    set(wrapper "${output}/gradlew.bat")
  else()
    set(wrapper sh "${output}/gradlew")
  endif()
  add_custom_target(${name}-apk
    COMMAND ${wrapper} -p "${output}" --no-daemon --max-workers=2

...
namespace Tempest {
  ...
  class AbstractGraphicsApi {
    ...
    public:
      ...
      class Props {
        public:
          ...
          struct {
            bool     supported = false; // VK_EXT_fragment_density_map with regular (non-subsampled) attachments
            uint32_t minTexel  = 0;     // pixels per density-map texel, per axis
            uint32_t maxTexel  = 0;
            } fragmentDensity;
      ...
      struct CommandBuffer:NoCopy {
        ...
        // Fragment density map for the following render passes (nullptr = none); ignored where unsupported.
        virtual void setFragmentDensityMap(Texture* map) { (void)map; }
        // Render area of the next render pass only (load/clear/store and the initial scissor);
        // an empty rectangle or an unsupporting backend keeps the full attachment.
        virtual void setRenderArea(const Rect& area) { (void)area; }
        ...
        virtual void drawIndexedIndirect(const Buffer& ibo, Detail::IndexClass cls,
                                         const Buffer& indirect, size_t offset, size_t count, size_t stride);
        virtual void drawIndexedIndirectCount(const Buffer& ibo, Detail::IndexClass cls,
                                              const Buffer& indirect, size_t offset,
                                              const Buffer& countBuffer, size_t countOffset, size_t maxCount, size_t stride);
    ...
    protected:
      ...
      virtual PTexture   createFragmentDensityMap(Device* d, const Pixmap& p) { (void)d; (void)p; return PTexture(); }

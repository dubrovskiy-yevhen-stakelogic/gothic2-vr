...
ShaderReflection::FragmentOutputs ShaderReflection::getFragmentOutputs(spirv_cross::Compiler& comp) {
  FragmentOutputs out;
  if(getExecutionModel(comp)!=Fragment)
    return out;
  ...
  for(const auto& r:resources.stage_outputs) {
    ...
    // Numeric scalar/vector and fixed arrays/matrices cover the current engine.
    // Unknown interface layouts fail closed at MRT pipeline creation, never guess.
    if(!comp.has_decoration(r.id,spv::DecorationLocation) ||
       type.basetype==spirv_cross::SPIRType::Struct || type.width>32 || type.vecsize>4) {
      out.known=false; continue;
      }
    uint32_t location=comp.get_decoration(r.id,spv::DecorationLocation);
    const uint32_t component=comp.has_decoration(r.id,spv::DecorationComponent) ?
        comp.get_decoration(r.id,spv::DecorationComponent) : 0;
    if(component+type.vecsize>4) {out.known=false;continue;}
    uint64_t count=type.columns;
    for(size_t i=0;i<type.array.size();++i) {
      if(!type.array_size_literal[i] || type.array[i]==0) {out.known=false;count=0;break;}
      count*=type.array[i];
      if(count>MaxFramebufferAttachments) {out.known=false;count=0;break;}
      }
    if(location+count>MaxFramebufferAttachments) {out.known=false;continue;}
    const auto mask=uint8_t(((1u<<type.vecsize)-1u)<<component);
    for(uint32_t i=0;i<count;++i)out.mask[location+i]|=mask;
    }
  return out;
  }

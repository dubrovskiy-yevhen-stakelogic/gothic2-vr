...
VCommandBuffer::~VCommandBuffer() {
  ...
  // VCommandPool releases every cached handle, including unused high-water slots.
  }
...
void VCommandBuffer::setFragmentDensityMap(AbstractGraphicsApi::Texture* map) {
  fdmMap = reinterpret_cast<VTexture*>(map);
  }

void VCommandBuffer::setRenderArea(const Rect& area) {
  nextRenderArea    = area;
  hasNextRenderArea = true;
  }

void VCommandBuffer::beginRendering(const FrameBufferDesc& fbo, size_t fboSize, uint32_t width, uint32_t height) {
  // One-pass render area (Encoder::setRenderArea), clamped to the attachments.
  // Dynamic rendering only: the render-pass emulation sizes its framebuffer
  // from the render area, so it keeps the full attachment.
  Rect area(0,0,int32_t(width),int32_t(height));
  if(hasNextRenderArea && device.props.hasDynRendering) {
    const int32_t x0 = std::max(0,std::min(nextRenderArea.x,int32_t(width)));
    const int32_t y0 = std::max(0,std::min(nextRenderArea.y,int32_t(height)));
    const int32_t x1 = std::max(0,std::min(nextRenderArea.x+nextRenderArea.w,int32_t(width)));
    const int32_t y1 = std::max(0,std::min(nextRenderArea.y+nextRenderArea.h,int32_t(height)));
    if(x1>x0 && y1>y0)
...

  {
    ...
    info.flags                = 0;
    info.renderArea.offset    = {area.x, area.y};
    info.renderArea.extent    = {uint32_t(area.w),uint32_t(area.h)};
    info.layerCount           = 1;
    info.viewMask             = 0;
    ...
    // VK_EXT_fragment_density_map: attach the map when its extent fits this
    // render area (between ceil(area/maxTexel) and ceil(area/minTexel)).
    VkRenderingFragmentDensityMapAttachmentInfoEXT fdmInfo = {};
    if(fdmMap!=nullptr && fdmMap->isDensityMap && device.props.fragmentDensity.supported && device.props.hasDynRendering) {
      const uint32_t mx = device.props.fragmentDensity.maxTexel, mn = device.props.fragmentDensity.minTexel;
      const bool ok = mx>0 && mn>0 &&
                      fdmMap->mapW>=(width +mx-1)/mx && fdmMap->mapW<=(width +mn-1)/mn &&
                      fdmMap->mapH>=(height+mx-1)/mx && fdmMap->mapH<=(height+mn-1)/mn;
      if(ok) {
        fdmInfo.sType       = VK_STRUCTURE_TYPE_RENDERING_FRAGMENT_DENSITY_MAP_ATTACHMENT_INFO_EXT;
        fdmInfo.imageView   = fdmMap->imgView;
        fdmInfo.imageLayout = VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT;
        fdmInfo.pNext       = info.pNext;
        info.pNext          = &fdmInfo;
        passDyn.fdm         = true;
        }
      }
  ...
  // All rendering must stay inside the render area: the scissor starts at it.
...
void VCommandBuffer::endRendering() {
  ...
  // Discover buffer dependencies before flushing. While still in RenderPass,
  // barrier() records them in the preceding chunk, before this pass executes.
...
void VCommandBuffer::drawIndexedIndirect(const AbstractGraphicsApi::Buffer& iibo, Detail::IndexClass cls,
                                         const AbstractGraphicsApi::Buffer& indirect, size_t offset, size_t count, size_t stride) {
  if(count>device.props.indirect.maxDrawCount || (count>1 && !device.props.indirect.multiDraw))
    throw std::system_error(Tempest::GraphicsErrc::UnsupportedExtension);
  const auto& ibo = static_cast<const VBuffer&>(iibo);
  const auto& ind = static_cast<const VBuffer&>(indirect);
  resState.onUavUsage(ind.nonUniqId,NonUniqResId::I_None,PipelineStage::S_Indirect);
  bindings.read |= ibo.nonUniqId;
  vkCmdBindIndexBuffer(impl,ibo.impl,0,nativeFormat(cls));
  implSetUniforms(PipelineStage::S_Graphics);
...
void VCommandBuffer::drawIndexedIndirectCount(const AbstractGraphicsApi::Buffer& iibo, Detail::IndexClass cls,
                                              const AbstractGraphicsApi::Buffer& indirect, size_t offset,
                                              const AbstractGraphicsApi::Buffer& countBuffer, size_t countOffset,
                                              size_t maxCount, size_t stride) {
  if(!device.props.indirect.count || device.vkCmdDrawIndexedIndirectCount==nullptr ||
     maxCount>device.props.indirect.maxDrawCount || (maxCount>1 && !device.props.indirect.multiDraw))
    throw std::system_error(Tempest::GraphicsErrc::UnsupportedExtension);
  const auto& ibo = static_cast<const VBuffer&>(iibo);
  const auto& ind = static_cast<const VBuffer&>(indirect);
  const auto& cnt = static_cast<const VBuffer&>(countBuffer);
  resState.onUavUsage(ind.nonUniqId | cnt.nonUniqId,NonUniqResId::I_None,PipelineStage::S_Indirect);
  bindings.read |= ibo.nonUniqId;
  vkCmdBindIndexBuffer(impl,ibo.impl,0,nativeFormat(cls));
  implSetUniforms(PipelineStage::S_Graphics);
  implSetPushData(PipelineStage::S_Graphics);
...
void VCommandBuffer::bindVbo(const VBuffer& vbo, size_t stride) {
  // Fixed-function vertex/index reads are absent from shader reflection.
  // Register them even when the native binding is cached for this draw.
  bindings.read |= vbo.nonUniqId;
  if(curVbo!=vbo.impl) {
...
void VCommandBuffer::setDebugMarker(std::string_view tag) {
  // On tile-based GPUs, timestamps inside a render pass can describe only one
  // tile or nearly zero elapsed time. Keep debug regions for those markers, but
  // measure GPU intervals only at the renderer's external pass boundaries.
  if(!profileLabels.empty() && state!=RenderPass) {
...
void VCommandBuffer::barrier(const AbstractGraphicsApi::SyncDesc& s, const AbstractGraphicsApi::BarrierDesc* desc, size_t cnt) {
  ...
  for(size_t i=0; i<cnt; ++i) {
    ...
    if(bx.newLayout==VK_IMAGE_LAYOUT_FRAGMENT_DENSITY_MAP_OPTIMAL_EXT) {
      bx.dstAccessMask |= VK_ACCESS_FRAGMENT_DENSITY_MAP_READ_BIT_EXT;
  ...
  if(state==RenderPass) {
    if(chunks.size()==0) {
      Chunk ch;
      // This late barrier must execute before the open first render pass. The
      // submission list stays independent of the cached allocation order.
      ch.impl = beginChunk();
      chunks.push(ch);
      }
...
VkCommandBuffer VCommandBuffer::beginChunk() {
  if(usedChunks==cachedChunks.size()) {
    // Reserve before Vulkan allocation so a host allocation failure cannot lose
    // the newly allocated handle from the cache.
    if(cachedChunks.size()==cachedChunks.capacity())
      cachedChunks.reserve(cachedChunks.empty() ? 8 : cachedChunks.size()*2);
    ...
    vkAssert(vkAllocateCommandBuffers(device.device.impl,&allocInfo,&cmd));
    cachedChunks.push_back(cmd);
    }
  auto cmd = cachedChunks[usedChunks];

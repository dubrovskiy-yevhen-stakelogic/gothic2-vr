...
class VCommandBuffer:public AbstractGraphicsApi::CommandBuffer {
  public:
    ...
    void setFragmentDensityMap(AbstractGraphicsApi::Texture* map) override;
    void setRenderArea(const Rect& area) override;
    ...
    void drawIndexedIndirect(const AbstractGraphicsApi::Buffer& ibo, Detail::IndexClass cls,
                             const AbstractGraphicsApi::Buffer& indirect, size_t offset, size_t count, size_t stride) override;
    void drawIndexedIndirectCount(const AbstractGraphicsApi::Buffer& ibo, Detail::IndexClass cls,
                                  const AbstractGraphicsApi::Buffer& indirect, size_t offset,
                                  const AbstractGraphicsApi::Buffer& countBuffer, size_t countOffset, size_t maxCount, size_t stride) override;
  ...
  protected:
    ...
    struct PipelineInfo:VkPipelineRenderingCreateInfoKHR {
      ...
      bool     fdm = false; // pass has a fragment density map attachment
      };
    ...
    // The command pool owns these handles. Reset only after the submission fence
    // completes; retain their storage across recordings and reuse up to high water.
    std::vector<VkCommandBuffer>             cachedChunks;
    size_t                                  usedChunks = 0;

    ResourceState                           resState;
    std::shared_ptr<VFramebufferMap::RenderPass> passRp;
    PipelineInfo                            passDyn = {};
    VTexture*                               fdmMap  = nullptr;
    Rect                                    nextRenderArea;          // consumed by the next beginRendering
    bool                                    hasNextRenderArea = false;

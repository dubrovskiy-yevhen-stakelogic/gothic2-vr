...
VDevice::VDevice(VkInstance instance, const bool hasDeviceFeatures2, uint32_t instanceApiVersion, VkPhysicalDevice pdev)
  :instance(instance), hasDeviceFeatures2(hasDeviceFeatures2),
    fboMap(*this), setLayouts(*this), psoLayouts(*this), descPool(*this) {
  deviceProps(instance, hasDeviceFeatures2, instanceApiVersion, pdev, props);
...
void VDevice::createLogicalDevice(VkPhysicalDevice pdev) {
  ...
  if(props.indirect.count)
    rqExt.push_back(VK_KHR_DRAW_INDIRECT_COUNT_EXTENSION_NAME);
  ...
  if(props.hasFragmentDensity) {
    rqExt.push_back(VK_EXT_FRAGMENT_DENSITY_MAP_EXTENSION_NAME);
    }
  ...
  if(hasDeviceFeatures2) {
    ...
    fdmFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_FEATURES_EXT;
    fdmFeatures.fragmentDensityMap                    = VK_TRUE;
    fdmFeatures.fragmentDensityMapNonSubsampledImages = VK_TRUE;
    ...
    if(props.hasFragmentDensity) {
      fdmFeatures.pNext = features.pNext;
      features.pNext = &fdmFeatures;
      }
    ...
    features.features.independentBlend=deviceFeatures.independentBlend;
  ...
  if(props.indirect.count) {
    // Enabling the KHR extension enables drawIndirectCount without adding the
    // Vulkan 1.2 feature aggregate (which conflicts with our extension structs).
    vkCmdDrawIndexedIndirectCount = PFN_vkCmdDrawIndexedIndirectCountKHR(vkGetDeviceProcAddr(device.impl,"vkCmdDrawIndexedIndirectCountKHR"));
    props.indirect.count = vkCmdDrawIndexedIndirectCount!=nullptr;
    }
...
void VDevice::deviceProps(VkInstance instance, const bool hasDeviceFeatures2, uint32_t instanceApiVersion, VkPhysicalDevice physicalDevice, VkProps& props) {
  const auto ext = extensionsList(physicalDevice);
  props.indirect.count = extensionSupport(ext,VK_KHR_DRAW_INDIRECT_COUNT_EXTENSION_NAME);
  ...
  if(hasDeviceFeatures2 && extensionSupport(ext,VK_EXT_FRAGMENT_DENSITY_MAP_EXTENSION_NAME)) {
    props.hasFragmentDensity = true;
    }
  ...
  props.render.independentBlend=(supportedFeatures.independentBlend==VK_TRUE);
  ...
  if(hasDeviceFeatures2) {
    ...
    // Core subgroup operations require Vulkan 1.1 on both the application and
    // device. OpenXR may cap the application's requested API independently.
    const bool hasCoreSubgroups = instanceApiVersion>=VK_API_VERSION_1_1 && devP.apiVersion>=VK_API_VERSION_1_1;
    VkPhysicalDeviceSubgroupProperties subgroupProperties = {};
    subgroupProperties.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SUBGROUP_PROPERTIES;
    if(hasCoreSubgroups) {
      subgroupProperties.pNext = properties.pNext;
    ...
    VkPhysicalDeviceFragmentDensityMapFeaturesEXT fdmQFeatures = {};
    fdmQFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_FEATURES_EXT;
    VkPhysicalDeviceFragmentDensityMapPropertiesEXT fdmQProps = {};
    fdmQProps.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_DENSITY_MAP_PROPERTIES_EXT;
    ...
    if(props.hasFragmentDensity) {
      fdmQFeatures.pNext = features.pNext;
      features.pNext = &fdmQFeatures;

      fdmQProps.pNext = properties.pNext;
      properties.pNext = &fdmQProps;
      }
    ...
    // Fragment density maps are used only with regular attachments (no subsampled images).
    props.hasFragmentDensity        = props.hasFragmentDensity && fdmQFeatures.fragmentDensityMap==VK_TRUE &&
                                      fdmQFeatures.fragmentDensityMapNonSubsampledImages==VK_TRUE;
    props.fragmentDensity.supported = props.hasFragmentDensity;
    props.fragmentDensity.minTexel  = std::max(fdmQProps.minFragmentDensityTexelSize.width, fdmQProps.minFragmentDensityTexelSize.height);
    props.fragmentDensity.maxTexel  = std::min(fdmQProps.maxFragmentDensityTexelSize.width, fdmQProps.maxFragmentDensityTexelSize.height);

    if(hasCoreSubgroups) {
      props.subgroup.size       = subgroupProperties.subgroupSize;
      props.subgroup.compute    = (subgroupProperties.supportedStages & VK_SHADER_STAGE_COMPUTE_BIT)!=0;
      props.subgroup.basic      = (subgroupProperties.supportedOperations & VK_SUBGROUP_FEATURE_BASIC_BIT)!=0;
      props.subgroup.arithmetic = (subgroupProperties.supportedOperations & VK_SUBGROUP_FEATURE_ARITHMETIC_BIT)!=0;
      }
  ...
  props.indirect.indexed       = true;
  props.indirect.multiDraw     = supportedFeatures.multiDrawIndirect;
  props.indirect.firstInstance = supportedFeatures.drawIndirectFirstInstance;
  props.indirect.maxDrawCount  = supportedFeatures.multiDrawIndirect ? devP.limits.maxDrawIndirectCount : 1;

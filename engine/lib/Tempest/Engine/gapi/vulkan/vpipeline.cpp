...
bool VPipeline::InstDr::isCompatible(const VkPipelineRenderingCreateInfoKHR& dr, VkPipelineLayout pLay, size_t stride, bool fdm) const {
  if(this->stride!=stride)
    return false;
  if(this->fdm!=fdm)
    return false;
...
VkPipeline VPipeline::initGraphicsPipeline(VDevice& device,
                                           ...
                                           const DSharedPtr<const VShader*>* shaders, bool fdm) {
  ...
  for(size_t i=0; i<size; ++i) {
    ...
    if(device.props.render.independentBlend && size>1) {
      const auto* fs=findShader(ShaderReflection::Fragment);
      if(fs!=nullptr && !fs->frag.known && size>1)
        throw std::runtime_error("Unsupported fragment output interface for multiple color attachments");
      if(fs==nullptr || fs->frag.known)
        a.colorWriteMask=fs==nullptr ? 0 : fs->frag.mask[i];
      VkFormatProperties formatProps{};
      vkGetPhysicalDeviceFormatProperties(device.physicalDevice,frm,&formatProps);
      if(a.colorWriteMask==0 || !(formatProps.optimalTilingFeatures & VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BLEND_BIT))
  ...
  if(fdm) {
    // Dynamic rendering with a fragment density map attachment
    pipelineInfo.flags |= VK_PIPELINE_CREATE_RENDERING_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_EXT;
    createFlags2.flags |= VK_PIPELINE_CREATE_2_RENDERING_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_EXT;
    }

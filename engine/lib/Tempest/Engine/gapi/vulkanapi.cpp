...
std::vector<AbstractGraphicsApi::Props> VulkanApi::devices() const {
  ...
  for(auto device : devices) {
    ...
    VDevice::deviceProps(impl->instance, impl->hasDeviceFeatures2, impl->instanceApiVersion, device, props);
...
AbstractGraphicsApi::Device* VulkanApi::createDevice(std::string_view gpuName) {
  ...
  for(const auto& device:devices) {
    ...
    if(!impl->isDeviceSuitable(device, props))
      continue;
    return new VDevice(impl->instance, impl->hasDeviceFeatures2, impl->instanceApiVersion, device);
    }

  throw std::system_error(Tempest::GraphicsErrc::NoDevice);
  }

VulkanNativeContext VulkanApi::nativeContext(Tempest::Device& source) {
  auto* d=dynamic_cast<VDevice*>(source.dev);
  if(d==nullptr || d->graphicsQueue==nullptr) throw std::runtime_error("Vulkan device required");
  return {d->instance,d->physicalDevice,d->device.impl,d->graphicsQueue->impl,
          d->graphicsQueue->family,&d->graphicsQueue->sync};
  }

VulkanNativeImage VulkanApi::nativeImage(const Tempest::Texture2d& source) {
  auto* image=dynamic_cast<VTexture*>(source.impl.handler);
  if(image==nullptr) throw std::runtime_error("Vulkan image required");
  return {image->impl,image->format,image->defaultLayout()};
  }

Attachment VulkanApi::borrowColorAttachment(Tempest::Device& source,const VulkanNativeImage& image,uint32_t width,uint32_t height) {
  auto* device=dynamic_cast<VDevice*>(source.dev);
  if(device==nullptr || image.image==VK_NULL_HANDLE || width==0 || height==0 ||
     width>device->props.tex2d.maxSize || height>device->props.tex2d.maxSize ||
     image.format!=VK_FORMAT_R8G8B8A8_UNORM || image.restingLayout!=VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)
    throw std::invalid_argument("Borrowed Vulkan attachment requires a valid RGBA8 color target");
  VTexture borrowed(*device,image.image);
  AbstractGraphicsApi::PTexture impl(new VTextureWithFbo(std::move(borrowed)));
  Texture2d texture(source,std::move(impl),width,height,1,TextureFormat::RGBA8);
  return Attachment(std::move(texture));
  }
...
AbstractGraphicsApi::PTexture VulkanApi::createFragmentDensityMap(AbstractGraphicsApi::Device* d, const Pixmap& p) {
  VDevice& dx = *reinterpret_cast<VDevice*>(d);
  if(!dx.props.fragmentDensity.supported || p.format()!=TextureFormat::RG8)
    return PTexture();

  const uint32_t size  = uint32_t(p.dataSize());
  VBuffer        stage = dx.allocator.alloc(p.data(),size,MemUsage::Transfer,BufferHeap::Upload);
  VTexture       tex   = dx.allocator.alloc(p,1,VK_FORMAT_R8G8_UNORM,true);

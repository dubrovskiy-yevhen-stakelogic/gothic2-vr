...
class VulkanApi : public AbstractGraphicsApi {
  public:
    ...
    static VulkanNativeContext nativeContext(Tempest::Device& device);
    static VulkanNativeImage nativeImage(const Tempest::Texture2d& texture);
    // Borrow a one-mip/layer/sample RGBA8 color target. Caller retains image/memory
    // ownership and guarantees COLOR_ATTACHMENT_OPTIMAL at each handoff.
    static Attachment borrowColorAttachment(Tempest::Device& device,const VulkanNativeImage& image,uint32_t width,uint32_t height);
  ...
  protected:
    ...
    PTexture       createFragmentDensityMap(Device* d, const Pixmap& p) override;

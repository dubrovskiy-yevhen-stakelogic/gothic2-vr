...
Texture2d Device::fragmentDensityMap(const Pixmap& pm) {
  if(!devProps.fragmentDensity.supported || pm.format()!=TextureFormat::RG8)
    return Texture2d();
  auto impl = api.createFragmentDensityMap(dev,pm);
  if(impl.handler==nullptr)
    return Texture2d();
  return Texture2d(*this,std::move(impl),pm.w(),pm.h(),1,TextureFormat::RG8);
  }

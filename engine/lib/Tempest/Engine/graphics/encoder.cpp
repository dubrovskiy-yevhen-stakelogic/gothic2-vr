...
void Encoder<Tempest::CommandBuffer>::setFragmentDensityMap(const Texture2d* map) {
  impl->setFragmentDensityMap(map!=nullptr ? map->impl.handler : nullptr);
  }

void Encoder<Tempest::CommandBuffer>::setRenderArea(const Rect& area) {
  impl->setRenderArea(area);
  }
...
static void validateIndexedIndirect(const StorageBuffer& indirect, size_t offset, size_t count, size_t stride) {
  constexpr size_t commandSize = sizeof(DrawIndexedIndirectCommand);
  static_assert(commandSize==20);
  if(offset%4!=0 || stride%4!=0 || stride<commandSize ||
     count>std::numeric_limits<uint32_t>::max() || stride>std::numeric_limits<uint32_t>::max() ||
     indirect.isEmpty() || offset>indirect.byteSize() || commandSize>indirect.byteSize()-offset ||
     count-1>(indirect.byteSize()-offset-commandSize)/stride)
    throw std::system_error(Tempest::GraphicsErrc::InvalidStorageBuffer);
  }

void Encoder<Tempest::CommandBuffer>::implDrawIndexedIndirect(const Detail::VideoBuffer& ibo, Detail::IndexClass cls,
                                                             const StorageBuffer& indirect, size_t offset, size_t count, size_t stride) {
  if(state.stage!=Rendering)
    throw std::system_error(Tempest::GraphicsErrc::DrawCallWithoutFbo);
  if(count==0)
    return;
  validateIndexedIndirect(indirect,offset,count,stride);
  if(!ibo.impl)
    throw std::system_error(Tempest::GraphicsErrc::InvalidStorageBuffer);
  impl->drawIndexedIndirect(*ibo.impl.handler,cls,*indirect.impl.impl.handler,offset,count,stride);
  }

void Encoder<Tempest::CommandBuffer>::implDrawIndexedIndirectCount(const Detail::VideoBuffer& ibo, Detail::IndexClass cls,
                                                                  const StorageBuffer& indirect, size_t offset,
                                                                  const StorageBuffer& countBuffer, size_t countOffset,
                                                                  size_t maxCount, size_t stride) {
  if(state.stage!=Rendering)
    throw std::system_error(Tempest::GraphicsErrc::DrawCallWithoutFbo);
  if(maxCount==0)
    return;
  validateIndexedIndirect(indirect,offset,maxCount,stride);
  if(!ibo.impl || countBuffer.isEmpty() || countOffset%4!=0 || countOffset>countBuffer.byteSize() ||
     sizeof(uint32_t)>countBuffer.byteSize()-countOffset)
    throw std::system_error(Tempest::GraphicsErrc::InvalidStorageBuffer);
  impl->drawIndexedIndirectCount(*ibo.impl.handler,cls,*indirect.impl.impl.handler,offset,
                                *countBuffer.impl.impl.handler,countOffset,maxCount,stride);
  }

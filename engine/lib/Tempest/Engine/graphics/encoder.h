...
class Encoder<Tempest::CommandBuffer> {
  public:
    ...
    // Fragment density map (fixed foveation) for the following render passes; nullptr disables it.
    void setFragmentDensityMap(const Texture2d* map);

    // Limits the next setFramebuffer() to this rectangle (clamped to the attachments):
    // the load/clear, the store and the initial scissor cover only it; pixels outside
    // keep their content. Applies to one render pass; empty = the whole attachment.
    void setRenderArea(const Rect& area);
    ...
    // Indexed indirect draws with an empty vertex buffer (shader vertex pulling).
    // Offsets and stride are bytes; GPU commands must obey the device's indirect caps.
    template<class I>
    void drawIndexedIndirect(const IndexBuffer<I>& ibo, const StorageBuffer& indirect, size_t offset,
                             size_t count=1, size_t stride=sizeof(DrawIndexedIndirectCommand)) {
      implDrawIndexedIndirect(ibo.impl,Detail::indexCls<I>(),indirect,offset,count,stride);
      }

    template<class I>
    void drawIndexedIndirectCount(const IndexBuffer<I>& ibo, const StorageBuffer& indirect, size_t offset,
                                  const StorageBuffer& countBuffer, size_t countOffset, size_t maxCount,
                                  size_t stride=sizeof(DrawIndexedIndirectCommand)) {
      implDrawIndexedIndirectCount(ibo.impl,Detail::indexCls<I>(),indirect,offset,countBuffer,countOffset,maxCount,stride);
      }
  ...
  private:
    ...
    void         implDrawIndexedIndirect(const Detail::VideoBuffer& ibo, Detail::IndexClass cls,
                                         const StorageBuffer& indirect, size_t offset, size_t count, size_t stride);
    void         implDrawIndexedIndirectCount(const Detail::VideoBuffer& ibo, Detail::IndexClass cls,
                                              const StorageBuffer& indirect, size_t offset,
                                              const StorageBuffer& countBuffer, size_t countOffset, size_t maxCount, size_t stride);

...
// VR 2002 static lighting: bit 0 marks a landscape pixel whose albedo already
// carries the Spacer bake (lighting/direct_light.frag applies the day curve only).
bool isGBufStaticLit(float v) {
  int x = int(v*255+0.5);
  return (x & 1)!=0;
  }

// VR: baked sun visibility of the landscape in the GBuffer hint bits 4-7 (15 = lit).
float gbufBakedSun(float v) {
  int x = int(v*255+0.5);
  return float((x >> 4) & 15)/15.0;
  }

...
vec3 luminance(vec3 norm) {
  ...
  return ambient;
#else
  return ambientLuminance(irradiance, scene.ambient, norm);
#endif
  }
...
void main() {
  ...
  const vec4  diff4 = texelFetch(gbufDiffuse, fragCoord, 0);
  const vec3  diff = diff4.rgb;
  const vec3  norm = normalFetch(gbufNormal, fragCoord);
  if(isGBufStaticLit(diff4.a)) {
    // 2002 static lighting: the sky term is inside the bake (direct_light.frag).
    outColor = vec4(0);
    return;
    }

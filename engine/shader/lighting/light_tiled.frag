...
void main() {
  ...
  for(uint i=0u;i<count;++i) {
    const LightSource source=lights[overflow?i:tiles[base+1u+i]];
    ...
    // Same unshadowed local-light equation as light.frag. Accumulate before
    // the single HDR blend, rather than repeatedly loading the GBuffer.

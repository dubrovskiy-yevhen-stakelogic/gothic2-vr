...
#if defined(INDEXED_OBJECTS)
#if defined(MAT_VARYINGS)
  bucketIdOut = bucketId;
#endif
  // The real index buffer contains absolute packed-vertex indices. Retain the
  // original vertex processing (skin/morph/wind/UVs), with hardware reuse.
  const uint vertexIndex = uint(gl_VertexIndex);
  Varyings var;
  vec4 pos = processVertex(var, instanceId, vertexIndex/MaxVert, bucketId, vertexIndex%MaxVert);
#else
...
void main() {
#if defined(GL_VERTEX_SHADER)
#if defined(INDEXED_OBJECTS) && !defined(BINDLESS)
  // Visibility reserves one contiguous, fixed-size meshlet group per object.
  const uint workIndex = uint(gl_InstanceIndex)*uint(push.meshletCount);
#else
...
#else
#if defined(INDEXED_OBJECTS) && defined(BINDLESS)
  // firstInstance in each native indirect command is the absolute payload slot.
  const uint firstMeshlet = 0;
#else

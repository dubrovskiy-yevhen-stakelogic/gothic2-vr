...
// Horizon haze (VR "Horizon haze"): geometry fades to the sky colour between
// the view depths push.hazeStart and push.hazeEnd (cm), so the far plane never
// shows as a hard edge. hazeEnd<=hazeStart disables it.
...
#if !defined(VOLUMETRIC)
  if(!isSky && push.hazeEnd>push.hazeStart) {
    const float dist = linearDepth(dMax, scene.clipInfo);
    const float f    = smoothstep(push.hazeStart, push.hazeEnd, dist);
    if(f>0.0) {

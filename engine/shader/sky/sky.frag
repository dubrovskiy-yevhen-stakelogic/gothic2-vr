...
#else
  // The slice fog.frag adds back for sky pixels: the old volume end dFogMax.
  vec3  maxFog = textureLod(fogLut, vec3(uv, fogVolumeSlice(linearDepth(dFogMax, scene.clipInfo), scene.clipInfo)), 0).rgb;
#endif
  // Sky

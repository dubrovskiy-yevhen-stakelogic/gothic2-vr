...
void main() {
  ...
  // Sky::isNight is exactly zero in daylight; the 32-step moon result then
  // contributes nothing. Keep every nonzero twilight/night value unchanged.
  vec3 moon = vec3(0.0);
  if(scene.isNight!=0.0)
    moon = raymarchScattering(viewPos, rayDir, normalize(vec3(-1,1,0)), tMax) * scene.isNight * (moonInt);

...
struct Npc {Vec3 pos;float facing=0;float rotationRad()const{return facing;}Vec3 position()const{return pos;}float qDistTo(const Item& item)const{return Vr::dot(item.pos-pos,item.pos-pos);}};
